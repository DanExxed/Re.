...
module.exports = async (client) => {
  ...
  setTimeout(async()=> {
  console.log(blue.bold(`[✅ ] :`)+`: Loaded the "Events" files`.green.bold)
  }, 200)
  ...
  setTimeout(async()=> {
  console.log(blue.bold(`[✅ ] :`)+`: Loaded the "SlashCommands" files`.green.bold)
  }, 200)
...
await client.application.commands.set(arrayOfSlashCommands).then((slashCommandsData) => {
  ...
  if(client.config.mongoDB.is_enabled == true) {
    ...
    mongoose.connect(process.env.mongoURL || client.config.mongoDB.mongoURL, {
    useUnifiedTopology: true,
    useNewUrlParser: true,
  }).then(console.log(blue.bold(`[✅ ] :`)+`: The MongoDB is CONNECTED`.green.bold)).catch(async(e)=> {
      console.log(blue.bold(`[⚠️ ] :`)+`: The MongoDB is HAVING ISSUES "Failed to Connect"`.yellow.bold)
  })
  } else {
    console.log(blue.bold(`[❌ ] :`)+`: The MongoDB is DISABLED`.red.bold)
  }
